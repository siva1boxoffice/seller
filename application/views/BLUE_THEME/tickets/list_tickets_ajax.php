 ...
 foreach ($listings as $list_ticket) {
               ...
               /* echo date('Y-m-d', strtotime($date. ' + 10 days')); 
                ...
                if($list_ticket->event_type == 'match'){
                  $listing_url = base_url().'tickets/index/listing_details/'.$list_ticket->m_id;
                }
                else{
                  $listing_url = base_url().'tickets/index/oe_listing_details/'.$list_ticket->m_id;
                }
...
<!-- <td><a href="javascript:void(0);" data-ticket-id="<?php echo $list_ticket->s_no; ?>" class="open_details"><img src="<?php echo base_url().THEME_NAME;?>/images/Vector.svg"></a></td>
 --><td class="entire_row" style="cursor: pointer;" data-href="<?php echo $listing_url;?>"><?php echo $list_ticket->match_name; ?> <!-- - <?php echo date('l', strtotime($list_ticket->match_date));?> <span>[<?php echo $list_ticket->ticket_group_id; ?>]</span> --><br><span><?php echo $list_ticket->country_name . ', ' . $list_ticket->city_name; ?></span><br><?php echo date('D d M Y', strtotime($list_ticket->match_date));?> <?php echo $list_ticket->match_time; ?> </td>
<td class="entire_row" style="cursor: pointer;" data-href="<?php echo $listing_url;?>"><span>Available Tickets:&nbsp;</span><?php echo (($total_qty != '') ? $total_qty : '0');?></td>
<td class="entire_row" style="cursor: pointer;" data-href="<?php echo $listing_url;?>"><span>Tickets sold:&nbsp;</span><?php echo (($list_ticket->sold_qty != '') ? $list_ticket->sold_qty : '0');?></td>
<td class="entire_row" style="cursor: pointer;" data-href="<?php echo $listing_url;?>"><span>Pending Fulfillment:&nbsp;</span> <?php echo (($list_ticket->pending_fullfillment != '') ? $list_ticket->pending_fullfillment : '0');?></td>
<td class="entire_row" style="cursor: pointer;" data-href="<?php echo $listing_url;?>"><?php echo $event_left;?></td>
<td><a href="<?php echo $listing_url;?>" class=""><img src="<?php echo base_url().THEME_NAME;?>/images/zoom.svg"></a></td>
</tr>
<?php } }else{ ?>
